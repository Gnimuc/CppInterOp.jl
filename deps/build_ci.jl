...
if isdir(joinpath(@__DIR__), "..", ".git")
    ...
    # find out which version of CppInterOpExtra_jll we are using
    Pkg.activate(joinpath(@__DIR__, ".."))
    deps = collect(values(Pkg.dependencies()))
    filter!(deps) do dep
        dep.name == "CppInterOpExtra_jll"
    end
    library_version = only(deps).version
    @info "CppInterOpExtra_jll version: $(library_version)"

    # compare to the JLL's tags
    jll_tags = mktempdir() do dir
        if !isdir(joinpath(support_dir, ".git"))
            run(`$(git()) clone -q https://github.com/JuliaBinaryWrappers/CppInterOpExtra_jll.jl $dir`)
        else
    ...
    jll_timestamp = jll_tags["CppInterOpExtra-v$(library_version)"]
    @info "CppInterOpExtra_jll timestamp: $(unix2datetime(jll_timestamp))"
